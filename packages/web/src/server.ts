...
async function render() {
  ...
  \${s.harnesses.map((h) => \`<tr>
    ...
    <td class="muted">\${h.installed ? (h.compatible ? "ready" : "dialect not served") : "not installed"}</td>
    <td><button data-run="\${esc(h.id)}" \${h.installed && h.compatible && s.selected.model ? "" : "disabled"}>launch</button></td></tr>\`).join("")}
  </table>\`;
  ...
  document.querySelectorAll("[data-run]").forEach((b) => b.onclick = async () => {
    ...
    if (r.script) alert("launch script:\\n\\n" + (r.printed || "opened in terminal"));
  });
    ...
    try {
      switch (url.pathname) {
        ...
        case "/api/run": {
          ...
          const res = await run({ harnessId: str("harness"), model, context: found?.contextWindow, vision: found?.vision });
          return json({ ok: true, script: res.script, harness: res.plan.harness.id });
        }
        default:
